...
- **Checkout:** `Checkout` (118:4743, "Confirm and Pay" — stepped payment flow + order summary; reuses `StarRating`; presentational) · `PaymentSuccess` (118:4814, "booking confirmed" success state + booking summary card).
...
- **Package detail** `src/app/packages/[id]/page.tsx` (`/packages/[id]`) — BUILT. Same design (Figma 141:3109) — REUSES `DestinationDetail` (passes `enquiryHref` → CTA "Enquire Now"; destinations reuse keeps "Explore Packages").
- **Package enquiry** `src/app/packages/[id]/enquiry/page.tsx` (`/packages/[id]/enquiry`) — BUILT (no Figma; built in the ContactForm/Checkout design language). `PackageEnquiry` section = no-payment enquiry form + package summary + inline success. This is the package-path reserve action (instead of `/checkout`, which stays as the payment page for navbar/CTA Reserve).
